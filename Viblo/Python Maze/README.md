...
ngồi ngắm 1 hồi thì mình biết được chương trình check từ ký tự của input qua `def s` vì check từng ký tự nên có  thể brute force từ ký tự được từ đó viết được [sờ cờ ríp](./script.py)
